...
export default async function PersonalContainersPage() {
  const accountId = await requireAccountId();

  const containers = await prisma.personContainer.findMany({
    where: { accountId },
  ...
  const [assetCounts, taskCounts, documentCounts] = await Promise.all([
    containerIds.length
      ? prisma.asset.groupBy({
          by: ["parentId"],
          where: { parentType: ParentType.PersonContainer, parentId: { in: containerIds } },
    ...
    <main className="container py-8">
      <div className="mb-6" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: ".5rem" }}>
          <Link href="/personal/new" className="btn btn-primary">New Personal Container</Link>
        </div>
      </div>
        ...
        <section className="card">
          <div className="card-content">
            <p className="text-muted-foreground" style={{ marginBottom: 8 }}>
              You don&apos;t have any personal containers yet.
            </p>
            <Link href="/personal/new" className="btn btn-primary">Create your first container</Link>
          </div>
        </section>
      ) : (
        <section className="card">
              ...
              <tbody>
                {containers.map((container) => {
                  ...
                  return (
                    <tr key={container.id}>
                      <td>
                        <Link href={`/personal/${container.id}`}>{container.label}</Link>
                      </td>
                      <td>
