// src/app/personal/[id]/edit/page.tsx
...
async function updatePersonalContainerAction(formData: FormData) {
    "use server";
    ...
    const existing = await prisma.personContainer.findFirst({ where: { id, accountId }, select: { id: true } });
    if (!existing) throw new Error("Container not found");

    await prisma.personContainer.update({
        where: { id },
        data: { label, notes },
    });
...
export default async function EditPersonalContainerPage({ params }: { params: { id: string } }) {
    const accountId = await requireAccountId();
    ...
    return (
        <main className="container py-8" style={{ maxWidth: 640 }}>
            <div className="mb-6" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <Link href={`/personal/${container.id}`} className="btn btn-outline">Back</Link>
            </div>

            <form action={updatePersonalContainerAction} className="card">
                ...
                <div className="card-content" style={{ display: "grid", gap: "1rem" }}>
                    ...
                    <div className="space-x-2">
                        <Link href={`/personal/${container.id}`} className="btn btn-outline">Cancel</Link>
