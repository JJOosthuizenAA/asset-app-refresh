...
export default async function OtherContainerDetailPage({ params }: { params: { id: string } }) {
  ...
  const [assets, tasks, documents] = await Promise.all([
    prisma.asset.findMany({
      where: { parentType: ParentType.OtherContainer, parentId: container.id },
    ...
    <main className="container py-8">
      <div className="mb-6" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: ".5rem" }}>
          <Link href={/other//edit} className="btn btn-outline">Edit</Link>
